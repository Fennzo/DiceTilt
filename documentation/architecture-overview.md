...
DiceTilt is a fully sovereign, locally-deployable hybrid Web2/Web3 system. All blockchain nodes run locally — no internet dependency, no real assets required. The user interacts entirely through a single browser endpoint (`localhost:80`).
...
graph TD
    ...
    subgraph "DiceTilt Platform — Docker Compose Stack"
        DT["All containerised services (TypeScript workers, Kafka, Redis, Postgres, Nginx)"]
    end
...
```mermaid
graph TD
    Client["frontend/index.html — Recruiter UI"]

    subgraph "API Ingress & Proxy Layer"
        Nginx["Reverse Proxy — Nginx — CORS, WS upgrade, static serve"]
        API["API / Gateway Service — TypeScript · Express · ws"]
    end

    subgraph "Messaging & Cache Layer"
        Redis[("Redis — Session · Balances · Nonce · Rate Limiter · Pub/Sub")]
        Kafka["Apache Kafka KRaft mode — Event Bus"]
        Postgres[("PostgreSQL — ACID Ledger")]
    end
    ...
    subgraph "Sovereign Web3 — Local EVM"
        EVMListen["EVM Listener Service — ethers.js · event subscription"]
        LocalEth(("Hardhat / Anvil — :8545"))
...
graph LR
    subgraph "Host Machine (published ports)"
        Browser["Browser — :80"]
        DevAPI["Direct API (load tests) — :3000"]
        GrafanaHost["Grafana — :3001"]
        PromHost["Prometheus — :9090"]
        EVMHost["Hardhat/Anvil — :8545"]
        RedisDebug["Redis (debug only) — :6380"]
    end

    subgraph "backend-net — single Docker bridge network"
        Nginx["Nginx — (host:80 → container:80)"]
        API["API Gateway — (host:3000 → container:3000) — Metrics aggregator: internal :9091"]
        PF["PF Worker — (internal :3001)"]
        Redis["Redis — (internal :6379; host:6380 for debug)"]
        Kafka["Kafka — (internal :29092)"]
        Postgres["PostgreSQL — (internal :5432)"]
